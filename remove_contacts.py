...
ag_move = UP.residues.atoms + u.select_atoms('resname TIP3 SOD CLA and prop z > %f' %halfz)
...
### Now that you make a bilayer, place a neutral lipid layer between.
u1 = mda.Universe('bilayer_moved.gro')
u2 = mda.Universe('neutral.gro')
u2.atoms.positions += xxx
new = mda.Merge(u2.atoms, u1.atoms)
new.atoms.write('combined.gro')

### Remove bad contacts
unew = mda.Universe('combined.gro')
ag = unew.select_atoms('not (same residue as (resname TRIO CHYO and around 2 (resname POPC DOPE SAPI)))')
ag.write('final.gro')